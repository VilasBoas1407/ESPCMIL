      ...
      <div class="row justify-content-center align-self-center">
        <div class="col-md-6 intro-info order-md-first order-last">
            <div class="form">
              ...
              <p></p>
              <form action="/admin/{{ session()->get('authAdmin')->nivel_acesso }}/{{ session()->get('authAdmin')->id }}/professor/add" method="post">
                <div class="form-group">
                  <label>Nome</label>
                  <input type="text" name="nome" class="form-control" id="nome" placeholder="" data-rule="minlen:4" data-msg="Campo inválido" />
                  <div class="validation"></div>
                </div>
                <div class="form-group">
                  <label>Telefone</label>
                  <input type="form-control" class="form-control" name="telefone" id="telefone" placeholder="" data-rule="required" data-msg="Campo inválido" /></input>
                  <div class="validation"></div>
                </div>
                <div class="form-group">
                  <label>E-mail</label>
                  <input type="form-control" class="form-control" name="email" id="email" placeholder="" data-rule="required" data-msg="Campo inválido" /></input>
                  <div class="validation"></div>
                </div>
                <div class="form-group">
                  <label>Senha</label>
                  <input type="password" class="form-control" name="senha" id="senha" placeholder="" data-rule="required" data-msg="Campo inválido" /></input>
                  <div class="validation"></div>
                </div>
                <div class="form-group">
                  <label>Materia Lecionada</label><br>
                  <select class="drop-down" id="materia" name="materia">
                    <option value="" >Selecione a opção certa</option>
                    @foreach($materia as $data)
                    <option value="{{$data->id_materia}}" >{{ $data->desc_materia }}</option>
                    @endforeach
                  </select>
                  <div class="validation"></div>
                </div>
